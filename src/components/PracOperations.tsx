...
export default function PracOperations({ userProfile }: { userProfile: UserProfile }) {
  ...
  const send = async () => {
    ...
    try {
      ...
      window.speechSynthesis?.cancel(); window.speechSynthesis?.speak(new SpeechSynthesisUtterance(body.answer));
